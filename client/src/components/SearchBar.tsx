...
function SearchBar() {
  ...
  useEffect(() => {
    if (
      ...
      fetch(
        `${import.meta.env.VITE_API_URL}/api/activity?name=${activityToPlay.sport}&city=${activityToPlay.city}&date=${activityToPlay.date}`,
      )
    ...
    <>
      <section className="searchbar">
        ...
        {!isMobile && (
          <button
            ...
            onClick={() => clickForSearchInDesktop()}
          >
            Rechercher
          </button>
        )}
      ...
      <section>
        {activities.length === 0 ? (
          ...
          <p>{activities.length} résultat</p>
        ) : (
